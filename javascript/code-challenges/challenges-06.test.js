...
const getNames = (arr) => {
  const names = [];
  arr.forEach(person => names.push(person.name));
  return names.map(person => person.split('').reduce((previousValue, currentValue) => currentValue + previousValue, ''));
};
...
const setStatusAsAuthor = (people) => {
  people.map(author => author.isAuthor = true);
};
...
// NOTE
// I also tried arr1 = [...arr1, ...arr2];
// but it didn't work, although it did console.log correctly.
