...
const allUpperCase = (arr) => {
  let output = [];
  arr.forEach(i => output.push(i.toUpperCase()));
  return output;
};
...
const greeting = (word) => {
  let shout = `${word.toUpperCase()}!`;
  return shout;
};

const speaker = (words, callback) => {
  let output = [];
  words.forEach(i => output.push(callback(i)));
  return output;
};
...
const createList = (availableItems) => {
  let output = [];
  availableItems.forEach(i => {if (i.available === true){ output.push(i.name);}});
  return output;
};
